...
class CompositeAdapter:
    ...
    async def read(self, tenant_id: str, memory_type: MemoryType, key: str) -> MemoryRecord | None:
        # L0: local LRU (< 0.1ms)
        record = await self._l0.read(tenant_id, memory_type, key)
        if record is not None:
            return record

        # L1: Redis (< 5ms)
        record = await self._l1.read(tenant_id, memory_type, key)
        if record is not None:
            await self._l0.write(tenant_id, memory_type, key, record.data, record.ttl_seconds)
            return record

        # L2: Java API (< 50ms)
        record = await self._l2.read(tenant_id, memory_type, key)
        if record is not None:
            # Backfill both caches
            await self._l1.write(tenant_id, memory_type, key, record.data, record.ttl_seconds)
            await self._l0.write(tenant_id, memory_type, key, record.data, record.ttl_seconds)
            return record
    ...
    async def write(self, tenant_id: str, memory_type: MemoryType, key: str,
        ...
        try:
            await self._l2.write(tenant_id, memory_type, key, data, ttl_seconds)
        except Exception as e:
            logger.warning(f"L2 write failed (will retry): {e}")

        # L1: Redis cache (fire-and-forget)
        try:
            await self._l1.write(tenant_id, memory_type, key, data, ttl_seconds)
        except Exception as e:
            logger.warning(f"L1 write failed: {e}")

        # L0: update local cache immediately
        record = await self._l0.write(tenant_id, memory_type, key, data, ttl_seconds)
        return record
    ...
    async def delete(self, tenant_id: str, memory_type: MemoryType, key: str) -> bool:
        deleted = False
        if await self._l0.delete(tenant_id, memory_type, key):
            deleted = True
        if await self._l1.delete(tenant_id, memory_type, key):
            deleted = True
        if await self._l2.delete(tenant_id, memory_type, key):
            deleted = True
        return deleted
    ...
    async def search(self, tenant_id: str, memory_type: MemoryType,
        ...
        if memory_type == MemoryType.META:
            return await self._l2.search(tenant_id, memory_type, query)
        # For LONG_TERM/SHORT_TERM, search L1 (Redis)
        results = await self._l1.search(tenant_id, memory_type, query)
        if not results:
            results = await self._l0.search(tenant_id, memory_type, query)
        return results

    async def exists(self, tenant_id: str, memory_type: MemoryType, key: str) -> bool:
        if await self._l0.exists(tenant_id, memory_type, key):
            return True
        record = await self.read(tenant_id, memory_type, key)
