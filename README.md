...
但是这种方案就会产生一些问题，首先迎面而来的就是记忆管理混乱，记忆管理就像打斗地主的时候不整理手里的牌一样，非常混乱。针对这个问题，主包设计了记忆树机制，同一个任务源会被归类到一个树下，并将这个树可视化，看着貌似还挺有意思。如果是思维比较发散的人用这个机制去学习，可能会真的会造出来一个树状的记忆。而且主包觉得在都在使用 AI Coding 的背景下，工作的全链路可追溯和将工作过程归纳成可复用资产变得尤为重要，记忆树机制对这些方面也同样有加持。

此外还有各种成本的产生，以及关于状态传递内容的考量等等，这也是最核心的挑战。
