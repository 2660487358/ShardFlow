...
async def test_l0_write_and_read():
    ...
    record = await adapter.write("user-1", MemoryType.SESSION_SUMMARY, "task-1",
                                  {"fact": "Dubbo uses ZooKeeper"})
    assert record.key == "task-1"
    assert record.data["fact"] == "Dubbo uses ZooKeeper"

    found = await adapter.read("user-1", MemoryType.SESSION_SUMMARY, "task-1")
...
async def test_l0_read_missing():
    adapter = L0CacheAdapter()
    found = await adapter.read("user-1", MemoryType.SESSION_SUMMARY, "nonexistent")
...
async def test_l0_user_isolation():
    adapter = L0CacheAdapter()
    await adapter.write("user-A", MemoryType.SESSION_SUMMARY, "task-1", {"data": "A"})
    await adapter.write("user-B", MemoryType.SESSION_SUMMARY, "task-1", {"data": "B"})

    a = await adapter.read("user-A", MemoryType.SESSION_SUMMARY, "task-1")
    b = await adapter.read("user-B", MemoryType.SESSION_SUMMARY, "task-1")
