...
class JavaAPIAdapter:
    ...
    async def write(self, tenant_id: str, memory_type: MemoryType, key: str,
        ...
        if memory_type == MemoryType.LONG_TERM:
            shard_data = {**data, "task_id": key, "tenant_id": tenant_id}
            try:
                await callback_client.save_shard(shard_data)
            except Exception:
                pass  # Degradation handled by CompositeAdapter
        elif memory_type == MemoryType.META:
            strategy_data = {**data, "strategy_id": key, "tenant_id": tenant_id}
            try:
    ...
    async def search(self, tenant_id: str, memory_type: MemoryType,
        ...
        if memory_type == MemoryType.META:
            try:
                ...
                return [
                    MemoryRecord(key=r.get("strategy_id", ""), tenant_id=tenant_id,
