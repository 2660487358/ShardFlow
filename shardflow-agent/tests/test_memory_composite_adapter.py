...
async def test_composite_read_l0_hit():
    ...
    # Write to L0
    await adapter._l0.write("u1", MemoryType.LONG_TERM, "k1", {"v": "L0"})

    record = await adapter.read("u1", MemoryType.LONG_TERM, "k1")
...
async def test_composite_write_broadcasts_to_l0():
    ...
    with (
        ...
        record = await adapter.write("u1", MemoryType.LONG_TERM, "k1", {"v": "written"})

    # Verify L0 has it immediately
    l0_record = await adapter._l0.read("u1", MemoryType.LONG_TERM, "k1")
...
async def test_composite_user_isolation_in_read():
    ...
    adapter = CompositeAdapter()
    await adapter._l0.write("user-A", MemoryType.LONG_TERM, "key-1", {"owner": "A"})
    await adapter._l0.write("user-B", MemoryType.LONG_TERM, "key-1", {"owner": "B"})

    a = await adapter._l0.read("user-A", MemoryType.LONG_TERM, "key-1")
    b = await adapter._l0.read("user-B", MemoryType.LONG_TERM, "key-1")
