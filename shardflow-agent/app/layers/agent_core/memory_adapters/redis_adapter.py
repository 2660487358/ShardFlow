...
Key format: "kb:{tenant_id}:mem:{memory_type.value}:{key}"
...
class RedisAdapter:
    ...
    def _build_key(self, tenant_id: str, memory_type: MemoryType, key: str) -> str:
        return f"kb:{tenant_id}:mem:{memory_type.value}:{key}"
    ...
    def _deserialize(self, raw: bytes | str) -> MemoryRecord:
        ...
        d = json.loads(raw)
        return MemoryRecord(
            key=d["key"], tenant_id=d.get("tenant_id", ""),
    ...
    async def read(self, tenant_id: str, memory_type: MemoryType, key: str) -> MemoryRecord | None:
        ...
        raw = await r.get(self._build_key(tenant_id, memory_type, key))
        if raw is None:
            return None
    ...
    async def write(self, tenant_id: str, memory_type: MemoryType, key: str,
        ...
        record = MemoryRecord(
            key=key, tenant_id=tenant_id, memory_type=memory_type,
            data=data, ttl_seconds=ttl_seconds, updated_at=now,
        )
        redis_key = self._build_key(tenant_id, memory_type, key)
    ...
    async def delete(self, tenant_id: str, memory_type: MemoryType, key: str) -> bool:
        r = await redis_client.get_redis()
        result = await r.delete(self._build_key(tenant_id, memory_type, key))
        return result > 0

    async def search(self, tenant_id: str, memory_type: MemoryType,
                     query: MemoryQuery) -> list[MemoryRecord]:
        r = await redis_client.get_redis()
        prefix = f"kb:{tenant_id}:mem:{memory_type.value}:"
        if query.key_prefix:
    ...
    async def exists(self, tenant_id: str, memory_type: MemoryType, key: str) -> bool:
        r = await redis_client.get_redis()
        return await r.exists(self._build_key(tenant_id, memory_type, key)) > 0
